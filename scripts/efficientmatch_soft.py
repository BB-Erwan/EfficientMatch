...
def run_efficientmatch():
    ...
    method_name = "efficientmatch_soft" + ("_flex" if adaptive_threshold else "") + ("_ema" if args.use_ema else "") + (f"_mu{mu}" if mu != 3 else "") + (f"_wf{args.widen_factor}" if args.widen_factor != 2 else "")
    ...
    for step in range(max_steps):
        ...
        with torch.no_grad():
            ...
            if adaptive_threshold:
                ...
                # bin 0 = count of still-unselected (-1) samples, bins 1..num_classes = per-class
                # counts. thresh_warmup relies on bin 0 dominating early training to keep
                # classwise_acc near 0 (permissive mask) until enough samples are confidently
                # pseudo-labeled -- it must NOT be filtered out beforehand.
