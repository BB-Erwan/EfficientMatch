"""Reusable accuracy-vs-time plotting tool for conference-style figures on this project.

Reads metrics JSON files directly (no training code involved) and plots test_acc vs
time_elapsed (in minutes) for one or more methods on a given config, styled for a
single-column conference figure (small figsize, large fonts, serif, 300dpi -- see
memory/publication-figure-style.md conventions).
...
all_methods_acc_vs_time_<dataset>_<num_labeled>_seed<seed>.png), matching the standing
...
def main():
    ...
    ap.add_argument("--xmax", type=float, default=None, help="Max x-axis (minutes). Defaults to the longest run's last time_elapsed.")
    ...
    ap.add_argument("--out", type=str, default=None, help="Output path. Defaults to figures/all_methods_acc_vs_time_<dataset>_<num_labeled>_seed<seed>.png")
    args = ap.parse_args()
    ...
    for label, fname in methods:
        ...
        with open(path) as f:
            d = json.load(f)
        time_min = [t / 60 for t in d["time_elapsed"]]
        acc = d["test_acc"]
        xmax = max(xmax, time_min[-1])
        ax.plot(time_min, acc, label=label, linewidth=1.3)
    ...
    out_path = args.out or os.path.join(FIGURES_ROOT, f"all_methods_acc_vs_time_{args.dataset}_{args.num_labeled}_seed{args.seed}.png")
