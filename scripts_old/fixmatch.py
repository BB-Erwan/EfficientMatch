...
def estimate_time_per_iter(args, device, warmup_iters=20, bench_iters=100):
    ...
    de chauffe non chronométrées -- nécessaires pour que cudnn.benchmark/l'allocateur CUDA se
    stabilisent) sur CIFAR-10 (respecte `args.debug_subset_size` pour rester rapide). Reproduit
    fidèlement la boucle de main() (mêmes appels, même option --amp), sans EMA ni
